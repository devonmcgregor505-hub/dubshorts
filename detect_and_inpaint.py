#!/usr/bin/env python3
"""
Optimized Auto Text Detection + LaMa Inpainting
Fast version for Mac with memory optimization
"""

import sys, json, subprocess, os, shutil
try:
    import keras_ocr
except ImportError:
    subprocess.check_call([sys.executable, "-m", "pip", "install", "keras-ocr", "-q"])
    import keras_ocr

def extract_frames(video_path, output_dir, max_frames=5):
    """Extract fewer frames for speed (5 instead of 10)"""
    os.makedirs(output_dir, exist_ok=True)
    
    # Extract 1 frame per 10 seconds instead of 5 seconds = 2x faster
    cmd = [
        'ffmpeg', '-i', video_path,
        '-vf', f'fps=1/10',  # Reduced from fps=1/5
        '-q:v', '2',
        os.path.join(output_dir, 'frame_%04d.jpg')
    ]
    
    subprocess.run(cmd, capture_output=True, check=True)
    frames = sorted([f for f in os.listdir(output_dir) if f.endswith('.jpg')])[:max_frames]
    return [os.path.join(output_dir, f) for f in frames]

def detect_text_boxes(frame_paths):
    """Detect text with memory optimization"""
    print("Loading Keras-OCR pipeline (this takes 30-60s first run)...")
    
    # Use lower resolution detection for speed
    pipeline = keras_ocr.pipeline.Pipeline()
    results = []
    
    for i, frame_path in enumerate(frame_paths):
        print(f"[{i+1}/{len(frame_paths)}] Detecting text in frame...")
        try:
            image = keras_ocr.tools.read(frame_path)
            
            # Downscale for faster detection
            h, w = image.shape[:2]
            if w > 1280:  # If wider than 1280px, downscale
                scale = 1280 / w
                new_h = int(h * scale)
                new_w = 1280
                # Use ffmpeg to resize (faster than Python)
                resized_path = frame_path.replace('.jpg', '_resized.jpg')
                subprocess.run([
                    'ffmpeg', '-i', frame_path, 
                    '-vf', f'scale={new_w}:{new_h}',
                    '-q:v', '2', resized_path
                ], capture_output=True, check=True)
                image = keras_ocr.tools.read(resized_path)
                scale_factor = 1 / scale
            else:
                scale_factor = 1
            
            predictions = pipeline.recognize([image])
            boxes = []
            
            for word_pred, box_pred in predictions[0]:
                if len(box_pred) == 4:
                    points = box_pred
                    x_min = min(p[0] for p in points)
                    y_min = min(p[1] for p in points)
                    x_max = max(p[0] for p in points)
                    y_max = max(p[1] for p in points)
                    
                    h, w = image.shape[:2]
                    box = {
                        'x': (x_min / w) / scale_factor,
                        'y': (y_min / h) / scale_factor,
                        'w': ((x_max - x_min) / w) / scale_factor,
                        'h': ((y_max - y_min) / h) / scale_factor,
                        'text': word_pred
                    }
                    
                    # Clamp to 0-1
                    box['x'] = max(0, min(1, box['x']))
                    box['y'] = max(0, min(1, box['y']))
                    box['w'] = min(1 - box['x'], box['w'])
                    box['h'] = min(1 - box['y'], box['h'])
                    
                    boxes.append(box)
            
            results.append((frame_path, boxes))
            print(f"  ✓ Found {len(boxes)} text regions")
            
            # Clear memory
            del image
            del predictions
            
        except Exception as e:
            print(f"  ⚠ Skipping frame: {str(e)[:50]}")
            results.append((frame_path, []))
    
    return results

def merge_boxes(detected_boxes, padding=0.05):
    """Merge all detected boxes with padding"""
    if not detected_boxes:
        return None
    
    x_mins = [max(0, b['x'] - padding) for b in detected_boxes]
    y_mins = [max(0, b['y'] - padding) for b in detected_boxes]
    x_maxs = [min(1, b['x'] + b['w'] + padding) for b in detected_boxes]
    y_maxs = [min(1, b['y'] + b['h'] + padding) for b in detected_boxes]
    
    merged_box = {
        'x': min(x_mins),
        'y': min(y_mins),
        'w': max(x_maxs) - min(x_mins),
        'h': max(y_maxs) - min(y_mins)
    }
    
    return merged_box

def inpaint_video_with_lama(video_in, video_out, detected_boxes):
    """Inpaint video with optimizations"""
    if not detected_boxes:
        print("No text detected. Copying video...")
        subprocess.run(['cp', video_in, video_out], check=True)
        return
    
    merged_box = merge_boxes(detected_boxes, padding=0.05)
    
    print(f"Inpainting region:")
    print(f"  x={merged_box['x']:.3f}, y={merged_box['y']:.3f}")
    print(f"  w={merged_box['w']:.3f}, h={merged_box['h']:.3f}")
    
    python_path = os.path.join(os.path.dirname(__file__), 'venv/bin/python')
    script_path = os.path.join(os.path.dirname(__file__), 'inpaint_captions.py')
    
    print("Starting inpainting (this may take 2-5 minutes)...")
    result = subprocess.run(
        [python_path, script_path, video_in, video_out, json.dumps(merged_box)],
        timeout=900,  # Increased timeout to 15 minutes
        capture_output=True
    )
    
    if result.returncode != 0:
        err = result.stderr.decode() if result.stderr else "Unknown error"
        raise Exception(f"Inpainting failed: {err[-500:]}")
    
    print("✓ Inpainting complete!")

def main():
    if len(sys.argv) < 3:
        print("Usage: python detect_and_inpaint.py <video_in> <video_out>")
        sys.exit(1)
    
    video_in = sys.argv[1]
    video_out = sys.argv[2]
    work_dir = '/tmp/text_detection_' + str(os.getpid())
    
    try:
        print(f"Input: {video_in}")
        print(f"Output: {video_out}\n")
        
        # Step 1: Extract key frames
        print("[Step 1/3] Extracting frames...")
        frame_paths = extract_frames(video_in, work_dir, max_frames=5)
        
        if not frame_paths:
            print("No frames extracted. Video may be too short.")
            subprocess.run(['cp', video_in, video_out], check=True)
            return
        
        print(f"Extracted {len(frame_paths)} frames\n")
        
        # Step 2: Detect text
        print("[Step 2/3] Detecting text...")
        detection_results = detect_text_boxes(frame_paths)
        
        all_boxes = []
        for frame_path, boxes in detection_results:
            all_boxes.extend(boxes)
        
        if all_boxes:
            print(f"\nTotal text regions found: {len(all_boxes)}\n")
        else:
            print("No text detected. Copying video as-is...")
            subprocess.run(['cp', video_in, video_out], check=True)
            return
        
        # Step 3: Inpaint video
        print("[Step 3/3] Inpainting video...")
        inpaint_video_with_lama(video_in, video_out, all_boxes)
        
        print("\n✓ All done!")
        
    except subprocess.TimeoutExpired:
        print("\n✗ Inpainting timed out (taking too long)")
        print("Your Mac may be running low on memory.")
        print("Try reducing video quality or length.")
        sys.exit(1)
        
    except Exception as e:
        print(f"\n✗ Error: {str(e)}")
        sys.exit(1)
        
    finally:
        # Cleanup
        if os.path.exists(work_dir):
            shutil.rmtree(work_dir, ignore_errors=True)

if __name__ == '__main__':
    main()
