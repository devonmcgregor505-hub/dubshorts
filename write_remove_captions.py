...
def get_ffmpeg():
    for cmd in ['ffmpeg', '/usr/bin/ffmpeg', '/opt/homebrew/bin/ffmpeg']:
        if shutil.which(cmd):
            return cmd
    return 'ffmpeg'
...
def to_numpy_bgr(result, reference_shape):
    """Convert LaMa result (PIL Image or numpy) to BGR uint8 numpy array."""
    # Handle PIL Image
...
def main():
    ...
    try:
        ...
        for i, fname in enumerate(frame_files):
            ...
            if use_lama:
                # LaMa expects RGB — convert BGR->RGB, pass in, convert result back
...
print('Done! remove_captions.py written successfully.')
