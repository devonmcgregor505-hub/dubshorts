...
app.post('/translate', upload.single('video'), async (req, res) => {
  ...
  try {
    // Step 1: Upload to tmpfiles for a public URL
    console.log('Step 1: Uploading to temp storage...');
    const uploadForm = new FormData();
    uploadForm.append('file', fs.createReadStream(videoPath), {
    ...
    // Step 2: Remove captions with Replicate
    console.log('Step 2: Removing captions with Replicate...');
    let cleanVideoUrl = videoUrl; // fallback to original if Replicate fails
    try {
      ...
      if (completed.status === 'succeeded' && completed.output) {
        ...
        if (typeof output === 'string') {
          cleanVideoUrl = output;
        } else if (Array.isArray(output) && output.length > 0) {
          cleanVideoUrl = typeof output[0] === 'string' ? output[0] : String(output[0]);
        } else if (output && typeof output === 'object') {
          cleanVideoUrl = output.url || output.video || output.output || videoUrl;
        }
      }
      console.log('Clean video URL:', cleanVideoUrl);
    } catch (repErr) {
      console.error('Replicate error (continuing with original):', repErr.message);
      cleanVideoUrl = videoUrl;
    }

    // Step 3: Send to ElevenLabs for dubbing
    ...
    while (status === 'dubbing' && attempts < 30) {
      ...
      attempts++;
      const checkRes = await axios.get('https://api.elevenlabs.io/v1/dubbing/' + dubbingId, {
    ...
    // Step 4: Download Spanish audio
    console.log('Step 4: Downloading Spanish audio...');
    const audioRes = await axios.get('https://api.elevenlabs.io/v1/dubbing/' + dubbingId + '/audio/es', {
      headers: { 'xi-api-key': process.env.ELEVENLABS_API_KEY },
      responseType: 'arraybuffer'
    });
    fs.writeFileSync(audioPath, audioRes.data);
    await axios.delete('https://api.elevenlabs.io/v1/dubbing/' + dubbingId, {
      headers: { 'xi-api-key': process.env.ELEVENLABS_API_KEY }
    }).catch(() => {});
