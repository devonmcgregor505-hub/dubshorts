...
function toAssTime(seconds) {
  const h=Math.floor(seconds/3600),m=Math.floor((seconds%3600)/60),s=Math.floor(seconds%60),cs=Math.floor((seconds%1)*100);
  return `${h}:${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}.${String(cs).padStart(2,'0')}`;
}
function srtToWordAss(srtContent,vidW,vidH) {
  const fontSize=Math.round(vidH*0.045),marginV=Math.round(vidH*0.05);
  const lines=['[Script Info]','ScriptType: v4.00+',`PlayResX: ${vidW}`,`PlayResY: ${vidH}`,'','[V4+ Styles]','Format: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, OutlineColour, BackColour, Bold, Italic, Underline, StrikeOut, ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, Encoding',`Style: Default,Arial,${fontSize},&H00FFFFFF,&H000000FF,&H00000000,&H00000000,1,0,0,0,100,100,0,0,1,4,0,2,10,10,${marginV},1`,'','[Events]','Format: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text'];
  for (const block of srtContent.trim().split(/\n\n+/)) {
    const blines=block.trim().split('\n');
    if(blines.length<3)continue;
    const timeLine=blines[1],textLine=blines.slice(2).join(' ').replace(/<[^>]+>/g,'').trim();
    if(!timeLine.includes('-->')||!textLine)continue;
    const [startStr,endStr]=timeLine.split('-->').map(t=>t.trim());
    const startSec=srtTimeToSeconds(startStr),endSec=srtTimeToSeconds(endStr);
    const words=textLine.split(/\s+/).filter(w=>w.length>0);
    if(!words.length)continue;
    const dur=(endSec-startSec)/words.length;
    for(let i=0;i<words.length;i++) lines.push(`Dialogue: 0,${toAssTime(startSec+i*dur)},${toAssTime(startSec+(i+1)*dur)},Default,,0,0,0,,${words[i]}`);
  }
  return lines.join('\n');
}
...
app.post('/translate', upload.single('video'), async (req, res) => {
  ...
  try {
    ...
    form.append('source_lang','en');form.append('target_lang','es');form.append('num_speakers','0');form.append('watermark','false');
    ...
    while(status==='dubbing'&&attempts<30){
      ...
      status=checkRes.data.status;console.log('ElevenLabs check '+attempts+': '+status);
    }
    ...
    try {
      ...
      if(srtRes.data&&srtRes.data.length>10){
        const assContent=srtToWordAss(srtRes.data,vidW,vidH);
        fs.writeFileSync(assPath,assContent);hasAss=true;
        console.log('ASS sample:',assContent.split('\n').filter(l=>l.startsWith('Dialogue')).slice(0,2).join(' | '));
      }
    ...
    // Schedule cleanup after 10 mins
