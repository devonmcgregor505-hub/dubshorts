  ...
  for (let i = 0; i < frames.length; i++) {
    ...
    if (captionMode === 'single') {
      ...
      lines.forEach((line, li) => {
        const lineW = line.reduce((acc, w) => acc + ctx.measureText(w.txt + ' ').width, 0);
...
app.post('/translate', upload.single('video'), async (req, res) => {
  ...
  try {
    const result = await enqueue(async () => {
      ...
      if (captionBox && req.body.removeCaption === 'true') {
      ...
      // Dub via ModelsLab using R2 upload
      if (targetLang === 'none') {
      ...
      if (targetLang === 'none') {
        ...
        // Put dubbed audio over original video (keeps original video quality, no glitching)
...
app.get('/test-audio', (req, res) => {
  const files = fs.readdirSync('uploads').filter(f => f.startsWith('dubbed_audio'));
  if (files.length === 0) return res.send('no audio files');
  const latest = files.sort().pop();
  res.setHeader('Content-Type', 'audio/mpeg');
  fs.createReadStream('uploads/' + latest).pipe(res);
});
